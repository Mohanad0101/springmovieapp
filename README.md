...
| Материал | Назначение |
|----------|------------|
| `README.md` | теория (§3–9) и пошаговая реализация (§10–18) |
| [`answers/otvety.md`](answers/otvety.md) | контрольные вопросы и примеры ответов |

**Условия:** создайте каталог `springmovieapp`, сформируйте Maven-проект (§11) и реализуйте классы по заданиям 12–18.

**ПО:** JDK 17+ (рекомендуется 21–23), Maven 3.9+, IntelliJ IDEA с плагином Lombok и включённой обработкой аннотаций (*Settings → Annotation Processors*).
...
| № | Действие | Раздел |
|---|----------|--------|
| 0 | Прочитать теорию | [§3–8](#3-теория-orm--класс-и-таблица-объект-и-строка) |
| 1 | Создать проект (Initializr) + **полный `pom.xml`** | [§11](#11-задание-1--создание-проекта-spring-initializr) |
| 2 | Открыть в IntelliJ, настроить JDK и Lombok | [§12](#12-задание-2--импорт-в-intellij-idea) |
| 3 | `application.properties` | [§13](#13-задание-3--настройка-h2-и-jpa) |
| 4 | `Movie.java` | [§14](#14-задание-4--сущность-movie) |
| 5 | `MovieRepository.java` | [§15](#15-задание-5--репозиторий) |
| 6 | `MovieService.java` | [§16](#16-задание-6--сервисный-слой) |
| 7 | `MovieController.java`, `HomeController.java` | [§17](#17-задание-7--rest-контроллер) |
| 8 | `MovieDataLoader.java` (начальные данные) | [§17а](#17а-задание-начальные-данные-moviedataloader) |
| 9 | `SpringmovieappApplication.java`, запуск, тесты | [§18](#18-задание-8--запуск-и-проверка) |
| 10 | Сверка по [чеклисту](#контрольный-список-файлов) | все 10 файлов на месте |

**Рекомендация:** после каждого задания выполняйте команду `mvn compile` или запуск приложения для раннего выявления ошибок.
...
| **Тема** | REST API каталога фильмов (`Movie`) на Spring Boot с ORM (JPA/Hibernate) |
| **Предварительные знания** | Java, Maven, SQL, JDBC, Hibernate (лабораторная работа 6) |
| **Время выполнения** | 4–6 академических часов |
| **Форма отчёта** | исходный код + скриншоты + ответы на вопросы |
| **Содержание** | трёхслойная архитектура; сущность `Movie` (название, жанр, год); H2 `jdbc:h2:mem:moviesdb`; маппинг JPA и сравнение с `.hbm.xml` из лабораторной 6 |

## Результаты обучения

По завершении работы студент **должен**:

| Категория | Результат |
|-----------|-----------|
| **Знать** | назначение Spring Framework, Spring Boot, Spring MVC, Spring Data JPA; различие спецификации JPA и реализации Hibernate; состояния сущности JPA; принципы IoC и DI |
| **Уметь** | создать Maven-проект, настроить `application.properties`, описать сущность `@Entity`, реализовать репозиторий, сервис и REST-контроллер; проверить API и выполнить SQL в консоли H2 |
| **Владеть** | навыками чтения SQL, генерируемого Hibernate (`show-sql`), и сопоставления операций `save` / `findById` / `delete` с командами INSERT / SELECT / DELETE |

**Обозначения:** артефакт Maven — `springmovieapp`; таблица — `movies`; REST API — `/api/movies`.
